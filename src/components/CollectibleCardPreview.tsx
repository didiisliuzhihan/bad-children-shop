...
export function CollectibleCardPreview({toy,item,active,onImageReady}:{toy:Toy;item:Capsule;active:boolean;onImageReady:()=>void}){
 ...
 return <article className={'collectible-live-card'+(toy.card_image_url?' has-art':'')} aria-label={tx(toy.name_zh+'收藏卡片')}>
  ...
  <div className="collectible-live-copy"><h3>{tx(toy.name_zh)}</h3><p className="collectible-live-english">{tx(toy.name_en)}</p><hr/><p>{tx(statement)}</p><p className="collectible-live-task">{tx(task.join('——').trim())}</p><time dateTime={item.obtained_at}>{tx(dateLabel(item.obtained_at))}</time></div>
 </article>;
}
