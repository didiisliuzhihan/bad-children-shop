...
export function CollectionPreview(){
 ...
 return <main className="shop is-entered preview-shop">
  <header className="topbar"><div className="brand"><BrandMark/><span>BAD CHILDREN<br/>SHOP</span></div><span className="preview-label">{tx("本地预览 · 不影响正式收藏")}</span><button className="icon-button" data-audio-toggle aria-label={tx(muted?'打开音乐':audioReady?'静音音乐':'开启声音')} onClick={()=>{const next=audioReady?!muted:false;setMute(next);setMuted(next);if(!next)void startBackground(asset('studio-loop.wav'))}}><Icon name={muted?'mute':'sound'}/></button></header>
  <Collection items={items} toys={toys} mode="local" onClose={()=>{location.href='http://127.0.0.1:4173/'}} toast={toast}/>
  {tx(initial&&<ToyRoom toy={popcorn} item={items[3]} onClose={close} toast={toast}/>)}
  <div className={'toast '+(message?'show':'')} role="status">{tx(message)}</div>
 </main>;
}
