 ...
 const language=useLanguage();
  ...
  const save=()=>{
    ...
    if(navigator.canShare?.({files:[ready.file]})&&matchMedia('(pointer:coarse)').matches){
      setSharing(true);void navigator.share({files:[ready.file],title:tx('我的扭蛋收藏')}).then(()=>onClose()).catch(error=>{if(error.name!=='AbortError')toast('可以长按预览图片，选择保存到相册。')}).finally(()=>setSharing(false));
    }else{
  ...
  return <Modal label={tx("保存收藏卡片")} onClose={onClose} className="save-modal">
    <h2>{tx("保存收藏卡片")}</h2>
    <div className="export-preview" aria-busy={!ready&&!error}>
      {tx(ready?<img src={ready.url} alt={tx(toy.name_zh+'收藏卡片预览')} width={1080} height={1440}/>:error?<div className="export-status"><p>{tx("图片或字体未能加载，请重试。")}</p><button className="pill-button dark" onClick={()=>setRetry(n=>n+1)}>{tx("重新生成")}</button></div>:<div className="export-status" role="status"><span className="loading-ring"/><p>{tx("正在准备完整图片…")}</p></div>)}
    </div>
    <p className="export-help">3:4 · 1080 × 1440<br/>{tx("也可以长按上方图片保存")}</p>
    <button className="pill-button dark export-save" disabled={!ready||sharing} onClick={save}><Icon name="download"/>{tx(sharing?'正在打开分享…':ready?'保存图片':'正在准备…')}</button>
  </Modal>;
}
