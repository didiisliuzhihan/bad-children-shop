...
export function NestDialogueBubble({bubble}:{bubble:NestBubble}){
 ...
 return <div className="nest-life-anchor" style={{left:bubble.x+'%',top:bubble.y+'%'}}>
  <button type="button" className="nest-life-bubble" onClick={()=>setExpanded(value=>!value)} aria-expanded={expanded} aria-label={tx(expanded?'收起小住客的话':'看看小住客想说什么')}>
   ...
   <span className="nest-life-line" aria-hidden={!expanded}><span><em>{tx(bubble.line)}</em></span></span>
  </button>
 </div>;
}
