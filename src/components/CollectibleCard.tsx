...
type Props={toy:Toy;item:Capsule;toast:(s:string)=>void;onConfirmSaved:()=>boolean|Promise<boolean>;questStage:string};
...
export function CollectibleCard({toy,item,toast,onConfirmSaved,questStage}:Props){
 ...
 const confirm=async()=>{
  if(!ready||sharing||confirming)return;setConfirming(true);
  if(await onConfirmSaved()){setSaveHint('任务出发啦，做完小事再回来看看它。');toast('做完小任务，回来敲敲玻璃吧。')}
  else setSaveHint('进度未能保存，请检查连接或浏览器存储设置后重试。');
  setConfirming(false);
 };
 return <div className="collectible-card-panel">
  ...
  <div className="card-save-actions"><button className="room-pill" disabled={!ready||sharing||confirming} onClick={save}><Icon name="download" size={17}/>{sharing?'正在打开分享…':'保存卡片'}</button><span>3:4 · 可长按保存</span></div>
  <div className="quest-confirm" data-confirm-state={questStage}>
   {questStage==='unsaved'?<><p>把卡片带走，再去完成那件小事。</p><button className="room-pill primary" disabled={!ready||sharing||confirming} onClick={()=>void confirm()}>{confirming?'正在保存进度…':'已保存，去做任务'}<Icon name="arrow" size={16}/></button></>:questStage==='waiting'?<><span className="quest-status-dot"/><p>小任务已经出发</p><small>做完小任务，去模型页敲敲玻璃吧。</small></>:<><Icon name={questStage==='unlocked'?'check':'heart'} size={17}/><p>{questStage==='unlocked'?'你们已经更亲近了一点':'小任务做完了么？去模型页看看它吧。'}</p></>}
  </div>
