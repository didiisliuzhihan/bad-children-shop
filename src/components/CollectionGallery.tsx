...
export function Collection({items,toys,mode,onClose,toast,communityPreview=false,production=false,playerNickname}:{items:Capsule[];toys:Toy[];mode:string;onClose:()=>void;toast:(s:string)=>void;communityPreview?:boolean;production?:boolean;playerNickname?:string}){
 ...
 return <section className={'collection-view'+(communityPreview?' community-collection':'')} aria-label={tx("我的扭蛋包")}>
  <header className="collection-header"><button className="text-button" onClick={onClose}><Icon name="back"/>{tx("返回机器")}</button><h1>{tx("我的扭蛋包")}<span>{tx(items.length+postcards.length)}</span></h1></header>
  {tx(communityPreview&&<div className="community-tabs" role="tablist" aria-label={tx("我的收藏空间")}>{tx(([['cards','卡包'],['nest','小窝'],['notes','彩蛋明信片']] as const).map(([key,label],index)=><button id={'community-tab-'+key} key={key} role="tab" aria-controls={'community-panel-'+key} aria-selected={tab===key} tabIndex={tab===key?0:-1} onClick={()=>setTab(key)} onKeyDown={e=>{const keys=['cards','nest','notes'] as const;let next:number|undefined;if(e.key==='ArrowRight')next=(index+1)%3;if(e.key==='ArrowLeft')next=(index+2)%3;if(e.key==='Home')next=0;if(e.key==='End')next=2;if(next!==undefined){e.preventDefault();setTab(keys[next]);document.getElementById('community-tab-'+keys[next])?.focus()}}}>{tx(label)}</button>))}</div>)}
  ...
  {tx(communityPreview&&<><section id="community-panel-nest" role="tabpanel" aria-labelledby="community-tab-nest" hidden={tab!=='nest'}><NestRoom soundActive={tab==='nest'} items={items} toys={toys} active={tab==='nest'&&!selected&&!story} onOpen={open} onCards={()=>setTab('cards')} onNotes={()=>setTab('notes')} playerNickname={playerNickname}/></section><section id="community-panel-notes" role="tabpanel" aria-labelledby="community-tab-notes" hidden={tab!=='notes'}><TicketStudio toast={toast} active={tab==='notes'&&!selected}/></section></>)}
  <footer className="collection-footer"><span><i className={'status-dot '+(mode==='cloud'?'online':'')}/>{tx(account?.persistence==='device'?'本地试玩 · 收藏仅保存在此浏览器':account?.profile?'云端已连接':communityPreview&&!production?'布局预览 · 使用示例收藏':mode==='cloud'?'云端已连接':'收藏保存在本机')}</span><span>{tx(account?.persistence==='device'?'不登录、不上传、不影响线上账户':account?.profile?'收藏与任务进度跟随账户':communityPreview&&!production?'本次布置不会改动线上收藏':'登录后，可把本机收藏带进账户')}</span></footer>
  {tx(selected&&<ToyRoom key={selected.toy.id} toy={selected.toy} item={selected.item} onClose={close} toast={toast} returnLabel={tx(communityPreview&&tab==='nest'?'回到小窝':'扭蛋包')}/>)}
  {tx(story&&<Modal label={tx(story.source==='nest'?'小窝生活明信片':'收到的彩蛋明信片')} onClose={()=>setStory(null)} className="nest-photo-modal"><Postcard source={story.source} text={story.text} date={story.createdAt} media={story.media} signature={story.source==='player'?story.signature:undefined} stamped={story.source==='player'}/></Modal>)}
 </section>;
}
