...
export function Collection({items,toys,mode,onClose,toast,communityPreview=false,production=false,playerNickname}:{items:Capsule[];toys:Toy[];mode:string;onClose:()=>void;toast:(s:string)=>void;communityPreview?:boolean;production?:boolean;playerNickname?:string}){
 ...
 return <section className={'collection-view'+(communityPreview?' community-collection':'')} aria-label="我的扭蛋包">
  ...
  <footer className="collection-footer"><span><i className={'status-dot '+(mode==='cloud'?'online':'')}/>{account?.persistence==='device'?'本地试玩 · 收藏仅保存在此浏览器':account?.profile?'云端已连接':communityPreview&&!production?'布局预览 · 使用示例收藏':mode==='cloud'?'云端已连接':'收藏保存在本机'}</span><span>{account?.persistence==='device'?'不登录、不上传、不影响线上账户':account?.profile?'收藏与任务进度跟随账户':communityPreview&&!production?'本次布置不会改动线上收藏':'登录后，可把本机收藏带进账户'}</span></footer>
