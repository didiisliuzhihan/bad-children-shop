...
export function TicketStudio({toast,active=true}:{toast:(message:string)=>void;active?:boolean}){
 ...
 return <div className="ticket-studio postcard-studio postcard-direct-studio" inert={saving} aria-busy={saving}>
  <header className="postcard-studio-heading"><div><h2>{tx("制作彩蛋明信片")}</h2><p>{tx("先留好一枚小彩蛋。现在可以保存私人草稿；投进公共扭蛋池、让别人抽到的功能暂未开放。")}</p></div><div className="notes-switch" role="group" aria-label={tx("彩蛋明信片工作台")}>{tx(([['write','制作彩蛋'],['drafts','我的草稿']] as const).map(([key,label])=><button key={key} aria-pressed={view===key} onClick={()=>{closeEditor();setView(key)}}>{tx(label)}{tx(key==='drafts'&&drafts.length>0&&' · '+drafts.length)}</button>))}</div></header>
  <div hidden={view!=='write'} className="postcard-direct-workbench">
   <p className="postcard-touch-hint">{tx("点图片、文字或印章，就能把它变成你的。")}</p>
   <section className="postcard-preview-pane" aria-label={tx("直接编辑彩蛋明信片")}>
   ...
   <div className="postcard-compose-actions"><button className="room-pill ticket-primary" disabled={saving||loading} onClick={()=>void savePlayer()}>{tx(saving?'正在保存…':'保存彩蛋草稿')}<Icon name="check" size={17}/></button></div>
  </div>
  <section hidden={view!=='drafts'} aria-label={tx("彩蛋明信片草稿夹")}>
   {tx(drafts.length?<div className="postcard-draft-grid">{tx(drafts.map(draft=><div key={draft.id} className="postcard-draft-item"><Postcard source={draft.source} text={draft.text} signature={draft.signature} media={draft.media} stamped date={draft.createdAt} active={active&&view==='drafts'}/><div className="postcard-draft-meta"><span>{tx(account?.profile?'账户草稿 · 未投进扭蛋池':'临时草稿 · 未投进扭蛋池')}</span><button className="text-button" disabled={saving} onClick={()=>void removeDraft(draft.id)}>{tx("移除草稿")}</button></div></div>))}</div>:<div className="community-empty"><Icon name="book" size={32}/><p>{tx("先做一枚能被偶然遇见的小彩蛋。")}</p><button className="room-pill" onClick={()=>setView('write')}>{tx("制作第一枚彩蛋")}</button></div>)}
  </section>
  {tx(!editor&&error&&<p className="postcard-compose-error" role="alert">{tx(error)}</p>)}
  <p className="postcard-save-disclosure">{tx(account?.persistence==='device'?'本地试玩：照片和草稿仅保存在此浏览器，不上传。':account?.profile?'保存时会上传所选影像，仅自己可见。':'未登录时只临时保存，刷新会清除；登录后可保存到账户。')}<br/>{tx("内测中：目前仅可存草稿，投进扭蛋池的功能尚未开放。")}</p>
  {tx(editor&&active&&<Modal className="postcard-edit-sheet" label={tx(editor==='media'?'编辑明信片图片':editor==='message'?'编辑明信片文字':editor==='signature'?'修改落款':'选择明信片印章')} onClose={closeEditor}>
   <span className="postcard-sheet-eyebrow">YOUR LITTLE SURPRISE</span>
   {tx(editor==='media'?<><h2>{tx("放一张你喜欢的画面")}</h2><div className="postcard-media-options"><button type="button" disabled={loading} onClick={()=>fileInput.current?.click()}><Icon name="camera" size={23}/><span><strong>{tx(loading?'正在准备影像…':'添加 / 更换影像')}</strong><small>{tx("照片、GIF 或静音短视频")}</small></span><Icon name="arrow" size={17}/></button><button type="button" onClick={()=>replaceMedia(null)}><Icon name="edit" size={23}/><span><strong>{tx("只留文字")}</strong><small>{tx("一句话也能成为小彩蛋")}</small></span><Icon name="arrow" size={17}/></button></div>
    <input ref={fileInput} type="file" accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/webm" hidden aria-label={tx("选择照片、GIF 或短视频")} onChange={e=>void selectFile(e.target.files?.[0])}/>
    <p className="postcard-sheet-help">{tx("照片 ≤ 12 MB · GIF ≤ 8 MB")}<br/>{tx("短视频 ≤ 15 秒 / 20 MB，实况请先导出 MP4。")}<br/>{tx("不要放地址、电话或私密照片。")}</p>
   </>:editor==='message'?<><h2>{tx("给抽到它的人，一句话")}</h2><form onSubmit={applyText}><div className="suggestion-heading"><span>{tx("选一句，再改改也可以")}</span><button type="button" className="text-button" onClick={()=>setSuggestions(old=>chooseNotes(old))}>{tx("换几句")}<Icon name="arrow" size={15}/></button></div><div className="note-suggestions">{tx(suggestions.map(note=><button type="button" key={note} aria-pressed={pendingText===tx(note)} onClick={()=>{setPendingText(tx(note));setError('')}}>{tx(note)}</button>))}</div>
    <label className="note-field"><span>{tx("或者，自己写")}</span><textarea value={pendingText} rows={3} maxLength={160} onChange={e=>{setPendingText(e.target.value);setError('')}} placeholder={tx("让某个还不认识的人，开心一下。")} aria-describedby="postcard-word-count postcard-edit-error"/></label><div className="note-field-foot"><span>{tx("不留联系方式，把偶遇留在扭蛋里。")}</span><span id="postcard-word-count">{tx(noteLength(pendingText))} / {tx(TICKET_LIMIT)}</span></div>
    <button type="submit" className="room-pill postcard-sheet-done">{tx("放到明信片上")}<Icon name="check" size={17}/></button></form>
   </>:editor==='signature'?<><h2>{tx("想留下什么名字？")}</h2><form onSubmit={e=>{e.preventDefault();setSignature(pendingSignature.trim());closeEditor()}}><label className="note-field"><span>{tx("明信片落款")}</span><input value={pendingSignature} maxLength={32} onChange={e=>setPendingSignature(Array.from(e.target.value).slice(0,16).join(''))} placeholder={tx("一个坏小孩")} autoComplete="off"/></label><p className="postcard-sheet-help">{tx("可以用昵称，也可以不留名；不会更改账户名字。")}</p><button type="submit" className="room-pill postcard-sheet-done">{tx("用这个落款")}<Icon name="check" size={17}/></button></form>
   </>:<><h2>{tx("给小彩蛋盖个戳")}</h2><p className="postcard-sheet-help">{tx("只有玩家制作的彩蛋明信片，才会盖上印章。")}</p><div className="postcard-stamp-options" role="group" aria-label={tx("已拥有的印章")}><button type="button" aria-pressed={stamped} onClick={()=>{setStamped(true);closeEditor()}}><img src={defaultStamp.imageUrl} alt={tx("羊女孩默认印章")}/><span><strong>{tx("羊女孩的戳")}</strong><small>{tx("默认赠送 · 点击盖上")}</small></span>{tx(stamped&&<Icon name="check" size={19}/>)}</button></div><p className="postcard-sheet-help">{tx("目前拥有 1 枚印章。")}</p></>)}
   <p id="postcard-edit-error" className="postcard-sheet-error" role="alert">{tx(error)}</p>
  </Modal>)}
 </div>;
}
