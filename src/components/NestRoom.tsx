...
function ResidentChoice({toy,item,count,placed,editing,selected,onEligibility,onPlace,onSelect,onOpen}:{toy:Toy;item:Capsule;count:number;placed:boolean;editing:boolean;selected:boolean;onEligibility:(id:string,allowed:boolean)=>void;onPlace:(id:string)=>void;onSelect:(id:string)=>void;onOpen:(toy:Toy,item:Capsule)=>void}){
 ...
 return <button className={'nest-resident-choice '+(!unlocked?'is-locked':'')} aria-pressed={selected} onClick={()=>!unlocked?onOpen(toy,item):placed?editing?onSelect(toy.id):onOpen(toy,item):onPlace(toy.id)}>
  <span className="nest-choice-image"><img src={toy.icon_url} alt="" loading="lazy"/>{tx(!unlocked&&<Icon name="lock" size={18}/>)}</span><span><strong>{tx(toy.name_zh)}</strong><small>{tx(!unlocked?'做任务，再入住':placed?'已在小窝':'放进小窝')}<span className="nest-owned-count">{tx(" · 卡包 ")}{tx(count)}{tx(" 枚")}</span></small></span>
 </button>;
}
export function NestRoom({items,toys,active=true,soundActive=active,onOpen,onCards,onNotes,playerNickname}:{items:Capsule[];toys:Toy[];active?:boolean;soundActive?:boolean;onOpen:(toy:Toy,item:Capsule)=>void;onCards:()=>void;onNotes:()=>void;playerNickname?:string}){
 ...
 return <div className={"nest-room nest-room-spatial"+(saving?" is-saving":"")} data-nest-time={timeOfDay} data-nest-active={active} data-nest-audio={audioStatus} inert={saving} aria-busy={saving}>
  <header className="nest-heading"><div><h2>{tx("我的小窝")}</h2><p>{tx(editing?'选中一位小住客，拖到喜欢的位置。':timeOfDay==='night'?'把灯留着，陪小住客待一会儿。':'陪小住客待一会儿。')}</p></div><div className="nest-mode-actions"><div className="nest-time-switch" role="group" aria-label={tx("小窝昼夜")}><button aria-pressed={timeOfDay==='day'} onClick={()=>setTimeOfDay('day')}>{tx("白天")}</button><button aria-pressed={timeOfDay==='night'} onClick={()=>setTimeOfDay('night')}>{tx("夜晚")}</button></div>{tx(editing?<><button className="room-pill" onClick={()=>{setPlacements(saved);setEditing(false);setSelected(null);setNotice('已回到上次保存的布置。')}}>{tx("取消")}</button><button className="room-pill nest-save" disabled={saving} onClick={()=>void save()}>{tx(saving?'正在保存…':'保存布置')}</button></>:<button className="room-pill" onClick={()=>{setEditing(true);setSelected(null);setNotice('')}}>{tx("布置小窝")}</button>)}</div></header>
  {tx(sceneVisited?<NestScene active={active} lifeReady={active&&!dirty&&(!account?.profile||baseRevision>0)&&!photo} toys={toys} placements={visible} editing={editing} selected={selected} timeOfDay={timeOfDay} onSelect={setSelected} onMove={move} onCapture={setPhoto}/>:<div className="nest-world nest-world-paused"/>)}
  <div className="nest-under-scene"><span>{tx(editing?'按住玩具拖动 · 空白处上下滑动':'逛小窝 · 有话想说时，点点「…」气泡')}</span><span>{tx(account?.profile?(dirty?'尚未保存':'已连接账户 · 布置需点保存'):`游客 · ${dirty?'尚未保存':persisted?'已保存在此浏览器':'点保存后留在此浏览器'} · 不跨设备同步`)}</span></div>
  {tx(active&&audioStatus==='loading'&&<p className="nest-life-status" role="status">{tx("炉火声正在加载…")}</p>)}
  {tx(active&&(audioStatus==='blocked'||audioStatus==='muted')&&<p className="nest-life-status" role="status">{tx(audioStatus==='muted'?'声音已关闭':'声音尚未开启')}{tx("，可点右上角喇叭打开。")}</p>)}
  {tx(active&&audioStatus==='error'&&<p className="nest-life-status" role="status">{tx("炉火声暂时没有加载成功。")}<button className="text-button" onClick={()=>void unlockAudio()}>{tx("重试炉火声")}</button></p>)}
  {tx(editing&&<section className="nest-placement-toolbar" aria-label={tx("调整玩具位置")}><div className="nest-selection-name">{tx(chosen?<><strong>{tx(chosen.toy.name_zh)}</strong><span>{tx("也可以用按钮微调位置")}</span></>:<span>{tx("在房间或下方选一位小住客。")}</span>)}</div>{tx(chosen&&<><div className="nest-nudge-buttons"><button onClick={()=>nudge(-.23,.195)}>{tx("左移")}</button><button onClick={()=>nudge(.23,-.195)}>{tx("右移")}</button><button onClick={()=>nudge(-.195,-.23)}>{tx("后移")}</button><button onClick={()=>nudge(.195,.23)}>{tx("前移")}</button></div><div className="nest-transform-buttons"><button onClick={()=>{setPlacements(p=>rotateResident(p,chosen.toy.id,Math.PI/12));setPersisted(false)}}>{tx("左转")}</button><button onClick={()=>{setPlacements(p=>rotateResident(p,chosen.toy.id,-Math.PI/12));setPersisted(false)}}>{tx("右转")}</button><button onClick={()=>{setPlacements(p=>p.filter(v=>v.toyId!==chosen.toy.id));setSelected(null);setPersisted(false);setNotice('已从房间收起，卡包收藏不变。')}}>{tx("收进卡包")}</button></div></>)}</section>)}
  <p className="nest-notice" role="status">{tx(notice)}</p>
  <section className="nest-resident-tray" aria-label={tx("可入住的玩具")}><div className="nest-tray-heading"><h3>{tx("小住客")}</h3><span>{tx(visible.length)}{tx(" 只在小窝 · 每款最多一只")}</span></div><div className="nest-resident-options">{tx(residents.map(({toy,item,count})=><ResidentChoice key={toy.id} toy={toy} item={item} count={count} placed={visible.some(p=>p.toyId===toy.id)} selected={editing&&selected===toy.id} editing={editing} onEligibility={eligibility} onPlace={add} onSelect={setSelected} onOpen={onOpen}/>))}</div>{tx(!residents.length&&<div className="community-empty"><p>{tx("第一位小住客，还在扭蛋机里等你。")}</p><button className="room-pill" onClick={onCards}>{tx("去看卡包")}</button></div>)}</section>
  <nav className="nest-shortcuts" aria-label={tx("小窝内的快捷入口")}><button onClick={onCards}><Icon name="book"/>{tx("打开卡册")}<Icon name="arrow" size={16}/></button><button onClick={onNotes}><Icon name="heart"/>{tx("制作彩蛋明信片")}<Icon name="arrow" size={16}/></button></nav>
  {tx(active&&photo&&<NestPhotoDialog photo={photo} playerNickname={playerNickname} onClose={closePhoto}/>)}
 </div>;
}
