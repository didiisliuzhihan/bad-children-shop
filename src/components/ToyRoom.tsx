  ...
  <div className="toy-room-layout">
   ...
   <section className="room-companion" hidden={mobile&&pane==='model'}>
    ...
    {pane==='story'&&<article id="room-panel-story" role="tabpanel" aria-labelledby={'room-'+(mobile?'mobile-':'desktop-')+'story'} className="room-story">
     ...
     <div className="room-story-copy"><h2>{toy.name_zh}</h2><p className="room-name-en">{toy.name_en}</p><p className="room-story-caption">{toy.story_text}</p>{toy.story_note&&<p className="room-story-note">{toy.story_note}</p>}{toy.audio_url?<button className="room-pill room-voice" aria-pressed={playing} onClick={listen}><Icon name={playing?'pause':'play'} size={17}/>{playing?'暂停故事':'听它说说话'}</button>:<p className="room-story-note">声音还在准备中，先读读它的小故事。</p>}</div>
    </article>}
   </section>
  </div>
 </Modal>;
