...
export function Modal({children,onClose,label,className=''}:{children:ReactNode;onClose:()=>void;label:string;className?:string}){
 ...
 return createPortal(<div className={`modal-backdrop ${className}`} onPointerDown={e=>{if(e.target===e.currentTarget)onClose()}}><div ref={ref} className="modal-panel" role="dialog" aria-modal="true" aria-label={tx(label)} tabIndex={-1}><button className="icon-button modal-close" onClick={onClose} aria-label={tx("关闭")}><Icon name="close"/></button>{tx(children)}</div></div>,document.body)
}
