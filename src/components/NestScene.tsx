...
export function NestScene(props:SceneProps){
 ...
 useEffect(()=>{
  ...
  const canvas=renderer.domElement;canvas.style.touchAction='pan-y pinch-zoom';canvas.dataset.nestRenderer=String(++rendererSequence);canvas.setAttribute('aria-label',tx('小窝三维空间；空白处上下滑动页面，布置时按住玩具拖动，也可以使用下方位置按钮'));el.append(canvas);
 ...
 useEffect(()=>{host.current?.querySelector('canvas')?.setAttribute('aria-label',tx('小窝三维空间；空白处上下滑动页面，布置时按住玩具拖动，也可以使用下方位置按钮'))},[language,retry]);
 useEffect(()=>api.current?.sync(),[props.placements,props.toys,props.editing,props.selected,props.timeOfDay,props.active]);
 return <><div className="nest-world" data-nest-mode={props.editing?'arrange':'visit'}><div ref={host} className="nest-world-canvas"/>{tx(mood&&!props.editing&&!bubble&&<NestStatusBubble key={mood.id} mood={mood}/>)}{tx(bubble&&!props.editing&&<NestDialogueBubble key={bubble.id} bubble={bubble}/>)}{tx(roomLoading&&!failure&&<div className="nest-world-status" role="status">{tx("正在为小窝亮灯…")}</div>)}{tx(loading>0&&!failure&&<div className="nest-world-status" role="status">{tx("小住客正在走过来…")}</div>)}{tx(failedIds.length>0&&!failure&&<div className="nest-world-status" role="status">{tx("有 ")}{tx(failedIds.length)}{tx(" 只暂时没加载好。")}<button className="room-pill" onClick={()=>setRetry(n=>n+1)}>{tx("重试模型")}</button></div>)}{tx((failure||roomFailure)&&<div className="nest-world-error" role="alert"><p>{tx("三维空间暂时没有打开，布置草稿仍在。")}</p><button className="room-pill" onClick={()=>setRetry(n=>n+1)}>{tx("重新打开")}</button></div>)}</div>{tx(tapNotice&&<p className="nest-life-status" role="status">{tx(tapNotice)}</p>)}{tx(life.error&&<p className="nest-life-status" role="status">{tx(life.error)}</p>)}{tx(props.onCapture&&<div className="nest-photo-actions"><span role="status">{tx(captureError|| (props.editing?'退出布置后，可以拍下这一刻。':''))}</span><button className="room-pill" disabled={capturing||props.editing||roomLoading||roomFailure||failure||loading>0||failedIds.length>0} onClick={()=>void takePhoto()}><Icon name="camera" size={18}/>{tx(capturing?'正在拍照…':'拍成明信片')}</button></div>)}</>;
}
