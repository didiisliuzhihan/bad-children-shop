...
function ToyImage({toy,story=false}:{toy:Toy;story?:boolean}){
 useLanguage();
  ...
  return failed?<span className="toy-image-fallback"><Icon name="heart" size={40}/><small>{tx(toy.name_zh)}</small></span>:<img src={story?toy.story_image_url:(toy.card_image_url||toy.icon_url)} loading={story?'eager':'lazy'} decoding="async" alt={tx(toy.name_zh)} onError={()=>setFailed(true)}/>;
}
...
export function Collection({items,toys,mode,onClose,toast}:{items:Capsule[];toys:Toy[];mode:string;onClose:()=>void;toast:(s:string)=>void}){
 useLanguage();
  ...
  return <section className="collection-view" aria-label={tx("我的扭蛋包")}>
