...
export function CollectedPostcardEntry({card,onOpen}:{card:CollectedPostcard;onOpen:()=>void}){
 const [failed,setFailed]=useState(false),player=card.source==='player';
 const thumbnail=card.media?.kind==='video'?card.media.posterUrl:card.media?.posterUrl||card.media?.url;
 return <button className="collected-postcard" onClick={onOpen} aria-label={'打开'+(player?'收到的彩蛋明信片':'小窝故事明信片')+'：'+card.text}>
  <span className="collected-postcard-photo">{thumbnail&&!failed?<img src={thumbnail} alt={player?'对方寄来的影像':'故事发生时的小住客'} loading="lazy" decoding="async" onError={()=>setFailed(true)}/>:<span className="collected-postcard-letter"><span>Postcard</span><small>{card.media?'这张照片暂时没能打开':'把一句话，好好收着。'}</small><i/><i/><i/></span>}{card.media&&card.media.kind!=='photo'&&<span className="collected-postcard-motion">{card.media.kind==='gif'?'GIF':'动态影像'}</span>}</span>
...
export function CollectionShelf({toyCards,toyCount,uniqueCount,postcards,onOpenPostcard,onDraw}:{toyCards:ReactNode;toyCount:number;uniqueCount:number;postcards:CollectedPostcard[];onOpenPostcard:(card:CollectedPostcard)=>void;onDraw:()=>void}){
 ...
 return <div className="collection-shelf">
  ...
  <section id="shelf-panel-postcards" role="tabpanel" aria-labelledby="shelf-tab-postcards" hidden={category!=='postcards'}>{postcards.length?<div className="collected-postcards-grid">{postcards.map(card=><CollectedPostcardEntry key={card.source+':'+card.id} card={card} onOpen={()=>onOpenPostcard(card)}/>)}</div>:<div className="collection-shelf-empty"><span className="empty-postcard-mark" aria-hidden="true">Postcard</span><h2>还没抽到明信片</h2><p>下一次转动，也许就有一小片生活等你。<br/>这里只收已抽到的明信片，不放创作草稿。</p><button className="room-pill primary" onClick={onDraw}>去抽扭蛋<Icon name="arrow" size={16}/></button></div>}</section>
 </div>;
}
