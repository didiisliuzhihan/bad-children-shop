...
export function CollectionShelf({toyCards,toyCount,uniqueCount,postcards,onOpenPostcard,onDraw}:{toyCards:ReactNode;toyCount:number;uniqueCount:number;postcards:CollectedPostcard[];onOpenPostcard:(card:CollectedPostcard)=>void;onDraw:()=>void}){
 ...
 return <div className="collection-shelf">
  ...
  <section id="shelf-panel-toys" role="tabpanel" aria-labelledby="shelf-tab-toys" hidden={category!=='toys'}>{tx(toyCount?<div className="collection-grid">{tx(toyCards)}</div>:<div className="collection-shelf-empty"><Icon name="bag" size={34}/><h2>{tx("第一位扭蛋宝，还在等你")}</h2><p>{tx("抽到的玩具卡会收在这里，重复的也保留。")}</p><button className="room-pill primary" onClick={onDraw}>{tx("去抽扭蛋")}<Icon name="arrow" size={16}/></button></div>)}</section>
  <section id="shelf-panel-postcards" role="tabpanel" aria-labelledby="shelf-tab-postcards" hidden={category!=='postcards'}>{tx(postcards.length?<div className="collected-postcards-grid">{tx(postcards.map(card=><CollectedPostcardEntry key={card.source+':'+card.id} card={card} active={category==='postcards'} onOpen={()=>onOpenPostcard(card)}/>))}</div>:<div className="collection-shelf-empty"><span className="empty-postcard-mark" aria-hidden="true">Postcard</span><h2>{tx("还没抽到明信片")}</h2><p>{tx("下一次转动，也许就有一小片生活等你。")}<br/>{tx("这里只收已抽到的明信片，不放创作草稿。")}</p><button className="room-pill primary" onClick={onDraw}>{tx("去抽扭蛋")}<Icon name="arrow" size={16}/></button></div>)}</section>
 </div>;
}
