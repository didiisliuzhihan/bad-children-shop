...
export default function App({preview}:{preview?:ShopPreview}={}){
  ...
  return <main className={'shop is-entered '+(bag?'has-bag ':'')+(overlay?'is-revealing':'')} data-community-preview={!!preview} data-phase={phase} data-audio-ready={audioReady}>
    ...
    {overlay&&drawResult?.type==='story'&&drawResult.story&&phase!=='SEALED'&&<section ref={revealRef} tabIndex={-1} className="story-reveal" role="dialog" aria-modal="true" aria-label="来自自己小窝的故事彩蛋"><Postcard source="nest" text={drawResult.story.text} date={drawResult.story.createdAt} media={drawResult.story.media}/><div className="decision-actions" style={{visibility:phase==='DECISION'?'visible':'hidden'}}><button className="pill-button dark" disabled={requesting} onClick={()=>void adopt()}>把这一刻收好<Icon name="heart"/></button><button className="pill-button" disabled={requesting} onClick={()=>void reject()}>先放回去</button></div></section>}
