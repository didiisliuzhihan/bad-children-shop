...
test('hidden home pauses rather than tearing down/recreating the scene and cannot reward hidden time',()=>{
 ...
 assert(scene.includes("state.active===false){cancelAnimationFrame(raf);raf=0;return;}"));assert(scene.includes('latest.current.active!==false'));assert(scene.includes('renderer.shadowMap.autoUpdate=false'));
