...
async function harness(type='toy'){
 ...
 const context={useCapsuleOpeningMotion:()=>({portraitRef:{current:null},rememberPose(){}}),canTransition,chooseToy,dragProgress,shouldCommitDrag,asset:s=>s,fallbackToys:[toy],loadModel:async()=>({scene:{}}),isAudioReady:()=>true,observeAudioReady:()=>()=>{},installAudioStart:()=>()=>{},sound:s=>sounds.push(s),startBackground:()=>{},muteAudio:()=>{},
 ...
 for(const name of ['Postcard','MachineScene','RevealScene','CapsuleSeam','BrandMark','Icon','Collection','Modal','Tagline'])context[name]=name;
 ...
 render();await flush();
 return {find,phase,render,flush,advance,sounds,draw,kept:()=>kept,rejected:()=>rejected,async spin(){find(n=>n.props?.['aria-label']==='轻点或向右滑动旋钮，或按回车抽取扭蛋').props.onClick({detail:0});await flush();await advance(5500)},open(){find(n=>n.type==='button'&&n.props.className==='seam-fallback').props.onClick();render()},unmount(){slots.forEach(s=>s?.cleanup?.())}};
}
...
test('model load completion before opening never advances the sealed phase; rapid double-open is idempotent',async()=>{
 const h=await harness();try{await h.spin();h.find(n=>n.type==='RevealScene').props.onReady();h.render();await h.advance(5000);assert.equal(h.phase(),'SEALED');const open=h.find(n=>n.type==='CapsuleSeam').props.onOpen;open();h.open();open();h.render();assert.equal(h.sounds.filter(s=>s==='open').length,1);await h.advance(1900);assert.equal(h.phase(),'DECISION');}finally{h.unmount()}
});
test('story capsules reach decisions without any model callback and never request a hidden toy',async()=>{
 const h=await harness('story');try{await h.spin();assert.equal(h.find(n=>n.type==='RevealScene').props.loadToy,false);h.open();await h.advance(600);assert.equal(h.phase(),'REVEALED');assert(h.find(n=>n.type==='RevealScene'));assert(!h.find(n=>n.type==='Postcard'));await h.advance(600);assert.equal(h.phase(),'DECISION');assert(h.find(n=>n.type==='Postcard'));}finally{h.unmount()}
});
