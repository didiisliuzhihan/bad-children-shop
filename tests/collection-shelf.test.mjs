...
async function load(hooks=React,jsx=jsxRuntime){const code=(await transformWithOxc(read('src/components/CollectionShelf.tsx'),'CollectionShelf.tsx',{jsx:{runtime:'automatic'}})).code.replace(/^import[^\n]*\n/gm,'').replace(/^export function /gm,'function ');const context={ResponsiveCardDeck:({children,gridClassName})=>React.createElement('div',{className:gridClassName},children),usePostcardMedia:media=>({media,ref:{current:null},loading:false,error:'',imageFailed(){},retry(){}}),...hooks,_jsx:jsx.jsx,_jsxs:jsx.jsxs,Icon:()=>null,defaultStamp:{id:'shop-default',name:'羊女孩默认戳',imageUrl:'/preview/default-stamp.png'}};vm.runInNewContext(code+'\nglobalThis.Shelf=CollectionShelf;globalThis.Entry=CollectedPostcardEntry;',Object.assign(context,localeMocks));return context;}
