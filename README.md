...
- 扭蛋包点开玩具默认显示完整卡片；手机可切换模型、卡片、故事，桌面左侧模型与右侧卡片/故事并列。
- 玩家主动确认「已保存，去做任务」后启动内部十分钟规则。界面只提示做完小任务回来，不显示等待时长或倒计时。确认前不启动、重复保存不重置，刷新保留同一浏览器进度。
- 回来后敲玻璃、回答任务提醒并擦去雾气，带轻柔音效解锁自由模型查看。锁定时不加载 GLB、不提供角度控件；抽中开蛋动画不受限制。
- 保存图片与预览使用同一份 1080 × 1440 PNG。保留 V5.3 双向滑轨动画与现有音量平衡，不改变匿名收藏或 RLS。
- 本机快速体验按钮仅在 loopback 主机且带专用预览标记时启用，线上不显示。模型解锁进度目前保存在浏览器，不是后端防作弊或跨设备同步功能。
- 源代码、原始素材仍全部保留；发布产物继续为单 HTML 入口，没有外部 JS/CSS 或大型二进制内联。
